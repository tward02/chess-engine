...
- `ApexNegamaxBot : EliteNegamaxBot` — the strongest bot; the anti-draw layer. Overrides `drawScore` with **contempt** (a draw scores `-contempt` for its own side, `+contempt` for the opponent, so it plays on in equal positions but still bails into a draw rather than lose more than `contempt`) and defaults to `EndgameConversionEvaluator`. The elite search is inherited unchanged.
